...
class Sumsub
  class << self
    ...
    APP_TOKEN = 'Some sumsub app token' # Example: tst:uY0CgwELmgUAEyl4hNWxLngb.0WSeQeiYny4WEqmAALEAiK2qTC96fBad
    SECRET_KEY = 'Some sumsub secret key' # Example: Hej2ch71kG2kTd1iIUDZFNsO5C1lh5Gq
    ...
    def pushApplicantMetadata(sumsub_user_id, user_wallet)
      resources = 'applicants/'
      req_body = {"id": "#{sumsub_user_id}", "metadata": [{"key": "walletAddress","value": "#{user_wallet}"}]}.to_json
      puts request_env_url(resources)
      response = RestClient.patch(request_env_url(resources), req_body, signed_header(resources, req_body, 'PATCH'))
  ...
  pushApplicantMetadata('5f9e7a83aee05c3aba7f956d', 'testwallet')
end
